...
module.exports = children => function (line) {
  ...
  if (!ok) {
    pauseChild(this, children, nextChildren.pid)
  }
}
...
const pauseChild = (parentStdin, children, childPid) => {
  paused[childPid] = true
  pausedCount++
  ...
  if (pausedCount === children.length) parentStdin.pause()

  nextChildren.stdin.once('drain', () => {
    paused[childPid] = false
    pausedCount--
